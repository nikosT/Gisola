...
module reflect5m
contains
subroutine reflect5(jf,nr,ns,globalbuffer)
...
!   write (out2, rec=base + 3) (uz(it), it=1, 6)
   ...
   do i=1,6
     globalbuffer(jf,i,ir,is)=ux(i)
     globalbuffer(jf,i+6,ir,is)=uy(i)
     globalbuffer(jf,i+12,ir,is)=uz(i)
   enddo
...
!   do i=1,6   
!      write(121,*) real(ux(i)),aimag(ux(i))
!   enddo
